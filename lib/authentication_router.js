...
router.use((req, res, next) => {
  res.redirectToIdP = function () {
    ...
    redirect(res, process.env.MODULE_IDENTITY_PROVIDER_URL +
      `/login?callbackUrl=${req.headers.host}${pathname}${pathname.endsWith('/') ? '' : '/'}consume_token`)
  }
...
router.get('/logout', (req, res) => {
  ...
  const host = parsedUrl.host || req.headers.host
  const pathname = parsedUrl.pathname.split('/').slice(-0, 1).join('/')
  redirect(res, process.env.MODULE_IDENTITY_PROVIDER_URL +
    `/logout?callbackUrl=${host}${pathname}/consume_token`)
})
