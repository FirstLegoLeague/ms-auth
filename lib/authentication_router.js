...
router.use((req, res, next) => {
  res.redirectToIdP = function () {
    const { pathname, host } = new url.URL(req.url)
...
router.use((req, res, next) => {
  if (publicRoutes.includes(new url.URL(req.url).pathname)) {
    return next()
  }
...
router.use('/consume_token', (req, res) => {
  const token = new url.URL(req.url, true).query.token || req.body.token
  if (!token) {
...
router.get('/logout', (req, res) => {
  ...
  const { host, pathname } = new url.URL(req.url)
  redirect(res, process.env.MODULE_IDENTITY_PROVIDER_URL +
